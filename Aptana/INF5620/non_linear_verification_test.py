...
def diffusion(dt,size):
    ...
    (-12*t*x[0] + 3*t*(-2*x[0]+3))*(   (x[0]*x[0]*x[0]*x[0])*(-dt+t)*(-dt+t)*(-2*x[0]+3)*(-2*x[0]+3) +36 )/324. -\
    ( -6*t*x[0]*x[0] + 6*t*x[0]*(-2*x[0]+3) )*( 36*x[0]*x[0]*x[0]*x[0]*(-dt+t)*(-dt+t)*(2*x[0]-3) + \
    36*x[0]*x[0]*x[0]*(-dt+t)*(-dt+t)*(-2*x[0]+3)*(-2*x[0]+3) )/5832. ',t = t, dt = dt, rho = rho)
    ...
    #plot(u)
    #u_a = Expression('t*x[0]*x[0]*(0.5-(1./3)*x[0])',t=10)
    u_e = interpolate(I, V)
    plot(u_e)
